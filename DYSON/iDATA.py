...
class iCORE50(CoRe):
    def __init__(self, root,
                 ...
                 download=False):
        super(iCORE50, self).__init__(root, train=train, transform=transform, target_transform=target_transform)
    ...
    def getGaussinTrain(self, filter_labels):
        datas, labels = [], []
        for l in filter_labels:
            data = self.data[np.array(self.targets) == l]
            datas.append(data)
            labels.append(np.full((data.shape[0]), l))
        self.TrainData, self.TrainLabels = self.concatenate(datas, labels)

    def getGaussinTest(self, filter_labels):
        datas, labels = [], []
...
class iCUB(CUB):
    def __init__(self, root,
                 ...
                 target_test_transform=None,
                 download=False):
        super(iCUB, self).__init__(root, train=train, transform=transform, target_transform=target_transform)
        self.target_test_transform = target_test_transform
        self.test_transform = test_transform
        self.TrainData = []
        self.TrainLabels = []
...
class iCIFAR10(CIFAR10):
    def __init__(self, root,
                 ...
                 target_test_transform=None,
                 download=False):
        super(iCIFAR10, self).__init__(root, train=train, transform=transform, target_transform=target_transform,
                                       download=download)
        self.target_test_transform = target_test_transform
        self.test_transform = test_transform
        self.TrainData = []
